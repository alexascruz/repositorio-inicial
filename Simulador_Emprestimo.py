...
def nova_tentativa():
    vlr_emprestimo = float(input("Qual o valor de emprestimo pretendido? "))
    qtd_meses = int(input("Em quantos meses pretende pagar? "))
    return vlr_emprestimo, qtd_meses
...
def calcula_consignavel (percentual_consignavel, salario):
    calculo_consignavel = salario * percentual_consignavel
    return calculo_consignavel

vlr_emprestimo, salario, qtd_meses = receber_valor()
vlr_consignavel = calcula_consignavel(percentual_consignavel, salario)

while True:

    vlr_final_emprestimo = calcula_emprestimo(vlr_emprestimo, taxa, qtd_meses)
    juros = vlr_final_emprestimo - vlr_emprestimo
    vlr_parcela = calcula_parcela(vlr_final_emprestimo, qtd_meses)
    ...
    if vlr_parcela <= vlr_consignavel:
        resposta = input("Parabéns! Simulação pré-aprovada. Deseja prosseguir com a contratação? ")

        if resposta.lower() == "sim":
            print("Certo. Você será direcionado para o próximo passo.")
            break
        else:
            print("Que pena! Volte aqui a qualquer momento.")
            break
    else:
        print(f"Lamentamos, simulação não foi aprovada. O valor da parcela não poderá ser superior à: {vlr_consignavel}")

        tentativa = input("Deseja fazer uma nova tentativa? ")

        if tentativa.lower() == "sim":
            vlr_emprestimo, qtd_meses = nova_tentativa()

        else:
            print("Ecerrando a simulação!")
            break